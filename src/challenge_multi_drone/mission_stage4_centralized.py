...
REPLAN_FREQUENCY = 20.0     # How often to replan (in Hz)
LOOKAHEAD_TIME = 1.5   # How far ahead to predict obstacle positions (in seconds)
...
CBS_CONSTRAINT_HORIZON = 50       # Maximum number of timesteps ahead to consider conflicts
...
class MultiDroneMission:
    def __init__(self, drone_ids, scenario_file, use_sim_time=True, verbose=False):
        ...
        self.cpu_samples = []
        self.path_lengths = {drone_id: 0.0 for drone_id in drone_ids}
        self.previous_positions = {}
        self.ideal_positions = {}
        self.max_deviation = 0.0
        self.min_clearance = float('inf')
        self.planning_times = []
        self.formation_errors = []  # Store all formation errors for RMS calculation
    ...
    def run_mission(self):
        ...
        try:
            ...
            for drone_id, drone in self.drones.items():
                ...
                # Initialize previous positions for path length calculation
                self.previous_positions[drone_id] = start_positions[drone_id].copy()
            ...
            try:
                while not all(drone_goal_reached.values()) and any(drone_active.values()):
                    ...
                    # Dictionary to store command parameters for each active drone - initialize here
                    drone_commands = {}
                    
                    # Get current positions for metrics tracking
                    current_positions = {}
                    for drone_id, drone in self.drones.items():
                        if drone_active[drone_id]:
                            current_positions[drone_id] = self.get_drone_position(drone)
                    
                    # Update path lengths
                    for drone_id, current_pos in current_positions.items():
                        prev_pos = self.previous_positions.get(drone_id)
                        if prev_pos:
                            distance = compute_euclidean_distance(prev_pos, current_pos)
                            self.path_lengths[drone_id] += distance
                            self.previous_positions[drone_id] = current_pos.copy()
                    
                    # Calculate ideal formation positions
                    if current_positions:
                        # Compute center of formation
                        ...
                        # Get ideal positions
                        ideal_positions = self.get_distributed_positions(center_position, len(self.drones))
                        self.ideal_positions = {drone_id: ideal_positions[i] for i, drone_id in enumerate(self.drones.keys())}
                        
                        # Calculate deviations
                        for drone_id, current_pos in current_positions.items():
                            ideal_pos = self.ideal_positions.get(drone_id)
                            if ideal_pos:
                                deviation = compute_euclidean_distance(current_pos, ideal_pos)
                                self.max_deviation = max(self.max_deviation, deviation)
                        
                        # Calculate and store formation error for RMS calculation
                        total_squared_error = 0.0
                        for drone_id, current_pos in current_positions.items():
                            ideal_pos = self.ideal_positions.get(drone_id)
                            if ideal_pos:
                                error = compute_euclidean_distance(current_pos, ideal_pos)
                                total_squared_error += error**2
                        
                        # Store the mean squared error at this time step
                        if current_positions:
                            mean_squared_error = total_squared_error / len(current_positions)
                            self.formation_errors.append(mean_squared_error)
                    
                    # Calculate minimum clearance
                    if current_positions and current_obstacles:
                        for drone_id, drone_pos in current_positions.items():
                            # Check clearance to obstacles
                            for obs_id, obs_data in current_obstacles.items():
                                obs_pos = obs_data["position"]
                                diameter = obs_data.get("diameter", 0.5)
                                # Calculate distance to obstacle center - radius
                                horizontal_dist = math.sqrt((drone_pos[0] - obs_pos[0])**2 + (drone_pos[1] - obs_pos[1])**2)
                                clearance = horizontal_dist - (diameter / 2.0)
                                if clearance > 0:  # Only consider positive clearances (no collisions)
                                    self.min_clearance = min(self.min_clearance, clearance)
                                
                                # Check clearance to other drones
                                for other_id, other_pos in current_positions.items():
                                    if other_id != drone_id:
                                        distance = compute_euclidean_distance(drone_pos, other_pos)
                                        self.min_clearance = min(self.min_clearance, distance)
                ...
                success = successful_drones > 0
            
            # Print metrics regardless of success or interruption
            self.print_metrics()
            
            return success  # Consider mission successful if at least one drone reached its goal
            
        except Exception as e:
            print(f"Error during mission execution: {e}")
            # Print metrics even in case of error
            self.print_metrics()
            return False
    
    def print_metrics(self):
        """Print all the collected metrics"""
        # Calculate mission time if completed
        mission_time = "N/A"
        if self.mission_start_time:
            current_time = time.time()
            if self.mission_end_time:
                mission_time = self.mission_end_time - self.mission_start_time
            else:
                # If mission_end_time is not set, use current time
                mission_time = current_time - self.mission_start_time
        
        # Format CPU time statistics
        if self.cpu_samples:
            cpu_time_total = sum(self.cpu_samples)
            cpu_time_avg = sum(self.cpu_samples)/len(self.cpu_samples)
            cpu_time_max = max(self.cpu_samples)
        else:
            cpu_time_total = 0
            cpu_time_avg = 0
            cpu_time_max = 0
        
        # Format path lengths with 2 decimal places
        path_lengths = [round(self.path_lengths[drone_id], 2) for drone_id in self.drones.keys()]
        
        # Format average replanning time
        avg_replanning_time = sum(self.planning_times)/len(self.planning_times) if self.planning_times else 0
        
        # Calculate RMS formation error
        rms_formation_error = 0.0
        if self.formation_errors:
            # RMS = sqrt(mean of squared errors)
            rms_formation_error = math.sqrt(sum(self.formation_errors) / len(self.formation_errors))
        
        # Print all metrics in the requested format
        print("\n======== MISSION METRICS ========")
        print(f"Mission time: {mission_time:.1f} s")
        print(f"CPU time: total={cpu_time_total:.3f} s, avg_loop={cpu_time_avg:.6f} s, max_loop={cpu_time_max:.6f} s")
        print(f"Average CPU Load: {cpu_time_avg:.6f}")
        print(f"Path length (per drone): {path_lengths} m")
        print(f"Max formation deviation: {self.max_deviation:.3f} m")
        print(f"RMS formation error: {rms_formation_error:.3f} m")
        print(f"Minimum clearance: {self.min_clearance:.3f} m")
        print(f"Average replanning time: {avg_replanning_time:.6f} s")
        print("=================================\n")
