...
def main():
    ...
    swarm.move_swarm(positions, altitude=altitude, speed=2.0)
    time0 = time.time()
    ...
    spread_mode_timer_active = False
    # spread_mode_exit_timer_active = False
    spread_mode_enter_time = None
    spread_mode_exit_time = None
    spread_mode_switch_time = None
    latency_mode = False
    # ───────────────────────────────
    ...
    for step in range(300):
        ...
        if not spread_mode_active and swarm_dia_x > active_window["gap_width"]:
            ...
            if not spread_mode_timer_active:
                spread_mode_timer_active = True
                spread_mode_enter_time = time.time()
                spread_mode_switch_time = time.time()  # For switch latency
                latency_mode = False
        ...
        if changed_phase:
            spread_mode_timer_active = False
            latency_mode = False
            spread_mode_exit_time = time.time()
            reform_duration = spread_mode_exit_time - spread_mode_enter_time
            reformation_times.append(reform_duration)
            ...
            for i in range(1, N + 1):
                ...
                # Move swarm
                swarm.move_swarm(intermediate_positions, altitude=intermediate_alt, speed=2.0)
        ...
        swarm.move_swarm(positions, altitude=active_window["alt"], speed=2.0)

        # Record switch latency as time between entering spread mode and first visible spread (dx > threshold)
        # if spread_mode_enter_time is not None and not latency_mode:
        if not spread_mode_timer_active and not latency_mode:
            # dxs = [abs(p[0] - base_wp[0]) for p in positions]
            # if max(dxs) > active_window["gap_width"]:
            # latency_mode = True
            lat = time.time() - spread_mode_enter_time
            switch_latency.append(lat)
            # spread_mode_enter_time = None  # Reset to prevent duplicates
            print(f"[Switch Latency] Detected: {lat:.2f} s")
            latency_mode = True
    ...
    avg_reformation_time = sum(reformation_times)/len(reformation_times) if reformation_times else 0
    avg_switch_latency = sum(switch_latency)/len(switch_latency) if switch_latency else 0
    avg_path = sum(path_length) / n_drones
    avg_cpu_load = sum(cpu_samples)/len(cpu_samples) if cpu_samples else 0

    # Display summary
    print(f"\n[METRICS SUMMARY]")
    print(f"Success: {1 if success else 0}")
    print(f"Total run time: {time_taken:.3f}s")
    print(f"Average reformation time: {avg_reformation_time:.3f}s")
    print(f"Average switch latency: {avg_switch_latency:.3f}s")
    print(f"Average path length: {avg_path:.3f}m")
    print(f"Average CPU Load: {avg_cpu_load:.3f}s/step")
    ...
    with open("results/stage2_dec.txt", "a") as f:
        f.write("---------------------------------------------------\n")
        f.write("Stage 2: Decentralized APF + Separation\n")
        f.write(f"Success: {1 if success else 0}\n")
        f.write(f"Total Time: {time_taken:.3f} s\n")
        f.write(f"Average Reformation Time: {avg_reformation_time:.3f} s\n")
        f.write(f"Average Switch Latency: {avg_switch_latency:.3f} s\n")
        f.write(f"Average Path Length: {avg_path:.3f} m\n")
        f.write(f"Average CPU Load: {avg_cpu_load:.3f} s/step\n")
    print("Metrics written to results/stage2_dec.txt")
