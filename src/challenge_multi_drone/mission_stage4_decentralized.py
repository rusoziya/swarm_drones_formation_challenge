...
# ORCA Parameters
ORCA_TIME_HORIZON = 6.0           # Time horizon for reciprocal collision avoidance (seconds)
ORCA_TIME_HORIZON_OBSTACLE = 4.0  # Time horizon for obstacle avoidance (seconds) - increased from 2.0 to 4.0
ORCA_RADIUS = 0.625                 # Radius of agent (meters)
ORCA_MAX_SPEED = 3            # Maximum speed of agent (m/s)
ORCA_NEIGHBOR_DISTANCE = 1.25      # Maximum distance to consider neighbors (meters)
ORCA_OBSTACLE_DISTANCE = 4.0      # Maximum distance to consider obstacles (meters) - increased from 6.0 to 8.0
FORMATION_WEIGHT = 0.1            # Weight of formation constraints (0-1) - reduced from 0.8 to 0.6 to prioritize obstacle avoidance

# System parameters
REPLAN_FREQUENCY = 10.0            # How often to replan (in Hz) - reduced to 5Hz to ensure control loop can finish
...
# Path length tracking (per drone)
path_lengths = {}

# Formation deviation tracking
formation_deviations = {}
max_formation_deviation = 0.0

# RMS formation error tracking
formation_squared_errors = []
rms_formation_error = 0.0

# Minimum clearance tracking
min_clearance = float('inf')

# ORCA planning time tracking
orca_planning_times = []

# Previous positions for path length calculation
previous_positions = {}
...
class ORCAPlanner:
    ...
    def compute_new_velocity(self):
        ...
        """
        # Track planning time
        planning_start_time = time.process_time()
        
        self.compute_preferred_velocity()
        
        lines = []
        
        # Only take the k closest neighbors
        neighbors = sorted(self.neighbors, key=lambda n: (n['position'].x - self.position.x)**2 + (n['position'].y - self.position.y)**2)[:4]
        
        # Track minimum clearance to neighbors
        global min_clearance
        ...
        if self.obstacles:
            # Only take the k closest obstacles
            obstacles = sorted(self.obstacles, key=lambda o: (o['position'].x - self.position.x)**2 + (o['position'].y - self.position.y)**2)[:6]
            
            # Track minimum clearance to obstacles
            for obstacle in obstacles:
                dist = math.sqrt((self.position.x - obstacle['position'].x)**2 + 
                                 (self.position.y - obstacle['position'].y)**2)
                effective_distance = dist - obstacle['radius'] - self.radius
                min_clearance = min(min_clearance, effective_distance)
        ...
        # Track planning time
        planning_time = time.process_time() - planning_start_time
        global orca_planning_times
        orca_planning_times.append(planning_time)
...
class MultiDroneMission:
    ...
    def orca_control_loop(self, drone_id, stop_flag):
        ...
        # Metrics tracking
        global path_lengths, previous_positions, formation_deviations, max_formation_deviation
        
        # Initialize path length for this drone
        if drone_id not in path_lengths:
            path_lengths[drone_id] = 0.0
            formation_deviations[drone_id] = []
        ...
        while not stop_flag.is_set():
            try:
                ...
                if drone_id in previous_positions:
                    prev_pos = previous_positions[drone_id]
                    distance = math.sqrt(
                        (current_position[0] - prev_pos[0])**2 +
                        (current_position[1] - prev_pos[1])**2 +
                        (current_position[2] - prev_pos[2])**2
                    )
                    path_lengths[drone_id] += distance
                
                # Store current position for next iteration
                previous_positions[drone_id] = current_position.copy()
                
                # Calculate formation deviation
                formation_pos = [
                    planner.formation_position.x,
                    planner.formation_position.y,
                    planner.goal_z if hasattr(planner, 'goal_z') else TAKE_OFF_HEIGHT
                ]
                
                deviation = math.sqrt(
                    (current_position[0] - formation_pos[0])**2 +
                    (current_position[1] - formation_pos[1])**2 +
                    (current_position[2] - formation_pos[2])**2
                )
                
                formation_deviations[drone_id].append(deviation)
                max_formation_deviation = max(max_formation_deviation, deviation)
                
                # Track squared error for RMS calculation
                global formation_squared_errors
                squared_error = (
                    (current_position[0] - formation_pos[0])**2 +
                    (current_position[1] - formation_pos[1])**2 +
                    (current_position[2] - formation_pos[2])**2
                )
                formation_squared_errors.append(squared_error)
    ...
    def end_mission(self):
        ...
        global mission_end_time, mission_time
...
def print_mission_metrics():
    """Print all metrics collected during the mission"""
    global mission_time, cpu_samples, path_lengths, max_formation_deviation, min_clearance, orca_planning_times
    global formation_squared_errors, rms_formation_error
    
    # Ensure mission_time has a value even if end_mission wasn't called
    if mission_start_time and not mission_time:
        mission_time = time.time() - mission_start_time
    
    # Calculate RMS formation error
    if formation_squared_errors:
        # RMS = sqrt(sum(squared_errors) / n)
        rms_formation_error = math.sqrt(sum(formation_squared_errors) / len(formation_squared_errors))
    
    # Format path lengths as a list
    path_lengths_list = [path_lengths.get(drone_id, 0.0) for drone_id in sorted(path_lengths.keys())]
    ...
    # CPU time metrics
    if cpu_samples:
        avg_cpu_time = sum(cpu_samples) / len(cpu_samples)
        max_cpu_time = max(cpu_samples)
        print(f"CPU time: total={sum(cpu_samples):.2f} s, avg_loop={avg_cpu_time:.6f} s, max_loop={max_cpu_time:.6f} s")
        print(f"Average CPU Load: {avg_cpu_time:.6f} s/step")
    else:
        print("CPU time: No samples collected")
    
    # Path length metrics
    if path_lengths:
        print(f"Path length (per drone): {[f'{length:.2f}' for length in path_lengths_list]} m")
    else:
        print("Path length: No data collected")
    
    # Formation metrics
    print(f"Max formation deviation: {max_formation_deviation:.4f} m")
    print(f"RMS formation error: {rms_formation_error:.4f} m")
    
    # Minimum clearance
    if min_clearance != float('inf'):
        print(f"Minimum clearance: {min_clearance:.4f} m")
    else:
        print("Minimum clearance: No data collected")
    
    # ORCA planning time
    if orca_planning_times:
        avg_planning_time = sum(orca_planning_times) / len(orca_planning_times)
        print(f"Average replanning time: {avg_planning_time:.6f} s")
    else:
        print("Average replanning time: No data collected")
...
if __name__ == '__main__':
    ...
    try:
        ...
        # Always print mission metrics, even if there was an error
        try:
            print_mission_metrics()
        except Exception as e:
