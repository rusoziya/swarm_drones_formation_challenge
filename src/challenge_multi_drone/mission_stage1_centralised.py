...
mission_end_time = 0
cpu_samples = []
path_lengths = {}  # Dictionary to store path length for each drone
drone_positions = {}  # Dictionary to track positions for each drone
max_formation_deviation = 0.0  # Maximum deviation from ideal formation
...
def update_path_length(drone_name, new_position):
    """Update the path length for a drone based on its new position."""
    global path_lengths, drone_positions
    
    # Initialize path length and positions if not already done
    if drone_name not in path_lengths:
        path_lengths[drone_name] = 0.0
    if drone_name not in drone_positions:
        drone_positions[drone_name] = new_position
        return
    
    # Calculate distance from last position
    distance = calculate_distance(drone_positions[drone_name], new_position)
    
    # Update path length and store new position
    path_lengths[drone_name] += distance
    drone_positions[drone_name] = new_position

def calculate_ideal_position(formation_idx, drone_idx, leader_pos, angle):
    """Calculate ideal formation position for a drone."""
    # Get formation offsets
    formation_func = None
    if formation_idx == 0:
        formation_func = formation_line_5
    elif formation_idx == 1:
        formation_func = formation_v_5
    elif formation_idx == 2:
        formation_func = formation_diamond_5
    elif formation_idx == 3:
        formation_func = formation_circular_orbit_5
    elif formation_idx == 4:
        formation_func = formation_grid_5
    elif formation_idx == 5:
        formation_func = formation_staggered_5
    ...
    # Leader is at index 2 (drone2)
    leader_offset = offsets[2]
    drone_offset = offsets[drone_idx]
    
    # Calculate base position (center of formation)
    base_x = leader_pos[0] - leader_offset[0]
    base_y = leader_pos[1] - leader_offset[1]
    
    # Apply rotation for the formation offset
    rotated = rotate_offset(drone_offset, angle)
...
def update_formation_deviation(drone_name, actual_position, formation_idx, leader_pos, angle):
    """Update the maximum formation deviation metric."""
    global max_formation_deviation
    
    # Map drone name to index
    drone_idx_map = {"drone0": 0, "drone1": 1, "drone2": 2, "drone3": 3, "drone4": 4}
    if drone_name not in drone_idx_map:
        return
    
    drone_idx = drone_idx_map[drone_name]
    
    # Calculate ideal position
    ideal_position = calculate_ideal_position(formation_idx, drone_idx, leader_pos, angle)
    if not ideal_position:
        return
    
    # Calculate deviation (Euclidean distance between actual and ideal)
    deviation = math.sqrt(
        (actual_position[0] - ideal_position[0])**2 + 
        (actual_position[1] - ideal_position[1])**2
    )
    
    # Update maximum deviation if this one is larger
...
class LeaderDrone(DroneInterface):
    ...
    def land_all(self, speed=0.4):
        ...
        global mission_end_time, mission_success
        ...
        # Record mission end time and success
    ...
    def execute_trajectory(self):
        ...
        while self.running and step < total_steps:
            ...
            new_position = (pos_x, pos_y, self.altitude)
            self.go_to_position(pos_x, pos_y, self.altitude, speed=move_speed)
            
            # Update leader's path length
            update_path_length(self.namespace, new_position)
        ...
        while self.running:
            ...
            if self.leader_state:
                ...
                if position and update_position:
                    ...
                    # Update path length for this drone
                    update_path_length(self.namespace, position)
                    
                    # Update formation deviation
                    if self.leader_state:
                        leader_pos = self.leader_state.get('position')
                        formation_idx = self.leader_state.get('formation')
                        angle = self.leader_state.get('angle')
                        update_formation_deviation(self.namespace, position, formation_idx, leader_pos, angle)
...
def main():
    global mission_success  # Move global declaration to the beginning of the function
    ...
    if args.role == 'leader':
        ...
        try:
            ...
            mission_success = False  # Remove global declaration
            print_performance_metrics()
        except Exception as e:
...
def print_performance_metrics():
    """Print a summary of all performance metrics at the end of the mission."""
    global mission_success, mission_start_time, mission_end_time, cpu_samples, path_lengths, max_formation_deviation
    ...
    # Mission time
    mission_time = mission_end_time - mission_start_time if mission_end_time > 0 else 0
    print(f"Mission time: {mission_time:.1f} s")
    
    # CPU time
    if cpu_samples:
        total_cpu_time = sum(cpu_samples)
        avg_loop_time = total_cpu_time / len(cpu_samples) if cpu_samples else 0
        max_loop_time = max(cpu_samples) if cpu_samples else 0
        print(f"CPU time: total={total_cpu_time:.3f} s, avg_loop={avg_loop_time:.6f} s, max_loop={max_loop_time:.6f} s")
    ...
    if path_lengths:
        drone_names = sorted(path_lengths.keys())
        lengths = [path_lengths[drone] for drone in drone_names]
        print(f"Path length (per drone): {lengths} m")
        print(f"  Details: {', '.join([f'{drone}: {path_lengths[drone]:.2f}m' for drone in drone_names])}")
    else:
        print("Path length: No data collected")
    
    # Maximum formation deviation
    print(f"Max formation deviation: {max_formation_deviation:.3f} m")
