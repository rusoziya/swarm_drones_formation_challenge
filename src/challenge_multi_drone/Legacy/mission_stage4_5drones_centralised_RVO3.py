...
class RVO:
    ...
    def _generate_candidates(self, current_velocity: Vector3, desired_velocity: Vector3):
        ...
        for _ in range(self.num_samples):
            # Biased sampling: 50% close to desired velocity, 50% uniformly
        ...
        # Add perpendicular samples for each obstacle
        for neighbor_pos, _ in neighbors:
            dx = neighbor_pos.x - position.x
            dy = neighbor_pos.y - position.y
            dist = math.sqrt(dx*dx + dy*dy)
            
            if dist > 0.1:  # Avoid division by near zero
                # Generate perpendicular directions (both left and right)
                perp1 = Vector3(
                    x=-dy/dist * self.max_speed * 0.8,
                    y=dx/dist * self.max_speed * 0.8,
                    z=desired_velocity.z
                )
                perp2 = Vector3(
                    x=dy/dist * self.max_speed * 0.8,
                ...
                candidates.append(perp1)
                candidates.append(perp2)
...
class Drone(DroneInterface):
    ...
    def __init__(self, namespace: str, config_path: str, verbose: bool = False, use_sim_time: bool = False):
        ...
        self.rvo = RVO(radius=0.6, time_horizon=1.5, max_speed=2.0, 
                       formation_weight=0.5, neighbor_weight=2.0, obstacle_weight=4.0)
    ...
    def calculate_position(self):
        ...
        try:
            ...
            direct_to_goal_factor = 0.6  # Reduce from 0.95 to 0.6
            ...
            min_speed = 0.3  # Reduced from 0.8 to allow more gradual movements
            if distance_to_goal > 0.5:  # Only apply minimum if not close to goal
                if abs(dx) < 0.001 and abs(dy) < 0.001:
                    ...
                    goal_speed = max(self._speed * 1.5, min_speed)  # Always using high speed
                    goal_vel = Vector3(
            ...
            obstacles = []
            for obs_id, obs_pos in self.dynamic_obstacles.items():
                obstacles.append([obs_pos.x, obs_pos.y])
            
            # Generate intermediate waypoint
            current_target = generate_intermediate_waypoint(
                [current_pos.x, current_pos.y], 
                [end_x, end_y], 
                obstacles
            )
            
            # Use intermediate target instead of end goal
            if current_target != [end_x, end_y]:
                # Use intermediate waypoint
                dx = current_target[0] - current_pos.x
                dy = current_target[1] - current_pos.y
            else:
                # Use final goal
                dx = end_x - current_pos.x
                dy = end_y - current_pos.y
...
def generate_intermediate_waypoint(leader_pos, target, obstacles, look_ahead_dist=1.5):
    """Generate intermediate waypoint to navigate around obstacles"""
    # If no obstacles or they're far away, head directly to goal
    if not obstacles:
        return target
    ...
    x, y = leader_pos
    tx, ty = target
    goal_dir = math.atan2(ty - y, tx - x)
    
    for obs in obstacles:
        ox, oy = obs
        # Check if obstacle is roughly in our path (within 45 degrees of goal direction)
        obs_dir = math.atan2(oy - y, ox - x)
        angle_diff = abs((obs_dir - goal_dir + math.pi) % (2 * math.pi) - math.pi)
        
        if angle_diff < math.pi/4:  # Within 45 degrees of our path
            d_obs = math.hypot(x - ox, y - oy)
            if d_obs < min_distance and d_obs < look_ahead_dist * 2:
                min_distance = d_obs
                closest_obstacle = obs
    
    if closest_obstacle and min_distance < look_ahead_dist:
        ...
        if dist < 0.1:  # Avoid division by near-zero
            ...
            vec_x, vec_y = vec_x / dist, vec_y / dist
        
        # Vector perpendicular to goal direction (choose the side that's better)
        perp1_x, perp1_y = -vec_y, vec_x  # One perpendicular direction
        perp2_x, perp2_y = vec_y, -vec_x   # The other perpendicular direction
        
        # Determine which perpendicular is better (closer to goal direction)
        perp1_goal_dot = perp1_x * (tx - x) + perp1_y * (ty - y)
        perp2_goal_dot = perp2_x * (tx - x) + perp2_y * (ty - y)
        
        if perp1_goal_dot > perp2_goal_dot:
            perp_x, perp_y = perp1_x, perp1_y
        else:
            perp_x, perp_y = perp2_x, perp2_y
        
        # Calculate intermediate waypoint (a safe distance away from obstacle)
        safe_distance = 1.0  # Adjust as needed
        waypoint_x = ox + perp_x * safe_distance
        waypoint_y = oy + perp_y * safe_distance
