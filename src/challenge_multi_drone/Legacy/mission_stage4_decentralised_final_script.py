...
ORCA_TIME_HORIZON_OBSTACLE = 2.0  # Time horizon for obstacle avoidance (seconds)
ORCA_RADIUS = 0.4                 # Radius of agent (meters)
ORCA_MAX_SPEED = 4             # Maximum speed of agent (m/s)
ORCA_NEIGHBOR_DISTANCE = 5.0      # Maximum distance to consider neighbors (meters)
ORCA_OBSTACLE_DISTANCE = 6.0      # Maximum distance to consider obstacles (meters)
FORMATION_WEIGHT = 0.2            # Weight of formation constraints (0-1)
...
class ORCAPlanner:
    ...
    def compute_new_velocity(self):
        ...
        # Create ORCA lines for obstacles with trajectory prediction - optimized with fewer points
        if self.obstacles:
            # Use reduced trajectory prediction for obstacles (3 points instead of 10+)
            for obstacle in self.obstacles:
                # Use only 3 key points (current, middle, end) for efficiency
                trajectory_points = [
        ...
        # Linear programming to find new velocity - implement robust LP solver
        ...
        # Add to velocity history (for debugging/analysis)
        self.velocity_history.append((new_velocity.x, new_velocity.y, vz))
        if len(self.velocity_history) > 100:  # Keep only last 100 velocities
    ...
    def robust_linear_program(self, lines, radius, optVelocity, result):
        ...
        if not lines:
            ...
            if optVelocity.length_squared() <= radius * radius:
                ...
                # Clamp to max speed
                normalized = optVelocity.normalize()
                result.x = normalized.x * radius
                result.y = normalized.y * radius
                return True
        
        # First try to use preferred velocity directly
        if optVelocity.length_squared() <= radius * radius:
            result.x = optVelocity.x
            result.y = optVelocity.y
            
            # Check if the preferred velocity satisfies all constraints
            for i in range(len(lines)):
                if cross(lines[i].direction, optVelocity - lines[i].point) < 0.0:
                    # Constraints not satisfied, need to use LP
                    break
        else:
                # All constraints satisfied
                return True
        
        # Standard linear program call
        if not linear_program2(lines, radius, optVelocity, False, result):
            ...
            if closest_line_idx >= 0:
                ...
                temp_result = Vector2()
                if linear_program1(lines, closest_line_idx, radius, optVelocity, temp_result):
...
class MultiDroneMission:
    def __init__(self, drone_ids, scenario_file, use_sim_time=True, verbose=False):
        ...
        for idx, drone_id in enumerate(drone_ids):
            ...
            # Set formation position (distributed end position)
        ...
        while not stop_flag.is_set():
            try:
                ...
                if elapsed >= control_rate:
                    ...
                    # Get only nearby obstacles once per cycle for efficiency
                    obstacles = self.obstacle_tracker.get_obstacles()
    ...
    def run_mission(self):
        ...
        # Set formation positions for each drone
        ...
        print("Starting ORCA control threads for all drones")
        for drone_id in self.drones:
