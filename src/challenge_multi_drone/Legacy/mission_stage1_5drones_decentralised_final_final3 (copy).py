...
class DecentralizedDroneNode(Node):
                ...
                if self.has_reached_target(position, self.target_position):
                    if not self.is_in_formation:
                        ...
                        if self.returning_home:
                            ...
                            if all_at_home:
                                self.get_logger().info("All drones have reached their home positions - ready to land")
                        else:
    ...
    def execute_landing(self):
        ...
        self.get_logger().info("All drones at home position, executing landing")
    ...
    def home_status_callback(self, msg, source):
        ...
        # Check if all drones have reached home position and we're at home too
        if self.returning_home and self.drone_home_status[self.drone_id] and self.check_all_drones_at_home():
            self.get_logger().info("All drones have reached home position, ready to land")
            
            # Only the drone with the lowest ID initiates landing to avoid multiple triggers
            drones_at_home = [d_id for d_id in self.expected_drone_ids 
                            if self.drone_home_status.get(d_id, False)]
            
            if drones_at_home and self.drone_id == min(drones_at_home):
                self.get_logger().info("I am the lowest ID drone at home - initiating landing for all")
