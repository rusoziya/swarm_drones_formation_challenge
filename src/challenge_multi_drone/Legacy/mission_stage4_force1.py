...
LOOKAHEAD_TIME = 2.25 # How far ahead to predict obstacle movement (seconds) 
...
CBS_MAX_ITERATIONS = 10 # Maximum iterations for CBS algorithm 
CBS_CONSTRAINT_HORIZON = 5 # Max timesteps ahead to consider conflicts 
CBS_SPATIAL_THRESHOLD = 1.5 # Maximum distance to consider conflicts 
CBS_MAX_WORKERS = 15 # Maximum number of worker threads for parallel planning 
CBS_CONFLICT_RESOLUTION_THRESHOLD = 0.5 # Accept solutions that resolve at least 80% of conflicts 
...
def world_to_grid_cached(point, low_bounds, resolution):
    """Cached version of world_to_grid conversion to avoid redundant calculations."""
...
def plan_astar(start, goal, dynamic_obstacles, bounds, resolution=ASTAR_RESOLUTION, 
    ...
    low_bounds = np.array([bounds["low"][0], bounds["low"][1], max(bounds["low"][2], MIN_ALTITUDE)], dtype=np.float32)
    high_bounds = np.array([bounds["high"][0], bounds["high"][1], min(bounds["high"][2], MAX_ALTITUDE)], dtype=np.float32)
    
    # Pre-compute grid dimensions
    grid_dims = np.round((high_bounds - low_bounds) / resolution).astype(np.int32)
    max_i, max_j, max_k = grid_dims
...
def detect_conflicts(paths, safety_distance=0.5, vertical_safety_distance=0.3, 
    ...
    Optimized with numpy vectorization for faster conflict detection.
    
    Modified to allow vertical stacking at same x,y coordinates when sufficient vertical separation exists.
    """
    conflicts = []
    
    # Get list of all drone IDs
    drone_ids = list(paths.keys())
    if len(drone_ids) < 2:  # No conflicts possible with fewer than 2 drones
        return conflicts
        
    # Determine maximum path length and limit to horizon
    max_path_length = min(max(len(paths[drone_id]) for drone_id in drone_ids), max_horizon)
    
    # Precompute squared safety distances for faster comparison
    safety_distance_sq = safety_distance**2
    
    # Process one timestep at a time to conserve memory
    for t in range(max_path_length):
        # Collect positions of all drones at this timestep
        positions = {}
        for drone_id in drone_ids:
            if t < len(paths[drone_id]):
                positions[drone_id] = paths[drone_id][t]
        
        # Skip timesteps with fewer than 2 drones
        if len(positions) < 2:
            continue
            
        # Convert to numpy arrays for vectorized processing
        drone_ids_at_t = list(positions.keys())
        positions_array = np.array([positions[d] for d in drone_ids_at_t], dtype=np.float32)
        
        # Apply spatial pruning if current positions are provided
        if current_positions:
            prune_mask = np.ones(len(drone_ids_at_t), dtype=bool)
            for i, drone_id in enumerate(drone_ids_at_t):
                if drone_id in current_positions:
                    curr_pos = np.array(current_positions[drone_id], dtype=np.float32)
                    dist = np.sqrt(np.sum((positions_array[i] - curr_pos)**2))
                    if dist > spatial_threshold:
                        prune_mask[i] = False
            
            # Skip this timestep if all drones are pruned
            if not np.any(prune_mask):
                continue
                
            # Filter positions and IDs to only include non-pruned drones
            positions_array = positions_array[prune_mask]
            drone_ids_at_t = [drone_ids_at_t[i] for i in range(len(drone_ids_at_t)) if prune_mask[i]]
            
            # Skip if fewer than 2 drones remain after pruning
            if len(drone_ids_at_t) < 2:
                continue
        
        # Compute all pairwise distances efficiently
        for i in range(len(drone_ids_at_t)):
            for j in range(i+1, len(drone_ids_at_t)):
                pos1 = positions_array[i]
                pos2 = positions_array[j]
                
                # Calculate horizontal distance (x,y only) using vectorization
                dx = pos1[0] - pos2[0]
                dy = pos1[1] - pos2[1]
                ...
                # Check for conflict based on both horizontal and vertical distances
                # MODIFIED: Allow vertical stacking by checking if they're at nearly the same x,y position
                if horizontal_dist_sq < safety_distance_sq:
                    # If drones are nearly stacked vertically (very close in x,y),
                    # only count as conflict if vertical separation is insufficient
                    is_vertical_stack = horizontal_dist_sq < (0.1 * safety_distance_sq)  # Within 10% of safety distance
                    
                    # For vertical stacks, require more separation
                    required_vertical_distance = vertical_safety_distance * 2.0 if is_vertical_stack else vertical_safety_distance
                    
                    if vertical_dist < required_vertical_distance:
                        # Conflict detected - create midpoint
                        midpoint = [(pos1[0] + pos2[0]) / 2, 
                                    (pos1[1] + pos2[1]) / 2, 
                                    (pos1[2] + pos2[2]) / 2]
                        
                        conflicts.append((drone_ids_at_t[i], drone_ids_at_t[j], midpoint, t))
...
def time_indexed_astar(start, goal, drone_id, constraints, dynamic_obstacles, bounds, 
    ...
    while open_set and iterations < max_iterations:
        ...
        for dx, dy, dz in directions:
            ...
            move_cost = 1.0  # Uniform cost for all moves
            ...
            if is_state_valid(next_world, predicted_obstacles, adjusted_pred_time):
                ...
                priority = new_cost + manhattan_distance(next_cell)
