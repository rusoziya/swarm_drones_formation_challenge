...
def cbs_plan_multi_drone_paths(drones, start_positions, goal_positions, dynamic_obstacles, bounds):
    ...
    """
    print("Planning multi-drone paths using optimized CBS")
    start_time = time.time()
    ...
    # If we reach here, CBS hit the iteration limit
    planning_time = time.time() - start_time
    print(f"CBS reached iteration limit of {max_iterations} after {planning_time:.3f}s")
    print(f"Returning best partial solution with {best_conflict_count} conflicts")
