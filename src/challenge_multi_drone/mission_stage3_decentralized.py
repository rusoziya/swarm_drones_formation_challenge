...
# Check if the current formation matches the default offsets
def is_default_formation(current_positions, formation_offsets, tolerance=0.2):
    """
    Check if the pairwise distances between drones match the default formation structure.
    """
    n = len(current_positions)
    for i in range(n):
        for j in range(i+1, n):
            # Expected relative distance in formation
            fx = formation_offsets[i][0] - formation_offsets[j][0]
            fy = formation_offsets[i][1] - formation_offsets[j][1]
            expected_dist = math.hypot(fx, fy)

            # Actual distance in current state
            dx = current_positions[i][0] - current_positions[j][0]
            dy = current_positions[i][1] - current_positions[j][1]
            actual_dist = math.hypot(dx, dy)

            if abs(expected_dist - actual_dist) > tolerance:
                return False
    return True
...
def main():
    ...
    for idx, center in enumerate(global_path):
        ...
        # Detect if it is in a non-default formation
        in_default_now = is_default_formation(safe_next_positions, base_formation)
        if not in_default_now and not in_nondefault_formation:
            # Started reformation
            metrics["formation_switch_count"] += 1
            print(f"[Formation Switch] Switched to non-default formation at waypoint {idx}. Total switches: {metrics['formation_switch_count']}")
            reform_start_time = time.time()
            in_nondefault_formation = True
        elif in_default_now and in_nondefault_formation:
            # Returned to default formation
            reform_duration = time.time() - reform_start_time
            metrics["reformation_times"].append(reform_duration)
            print(f"[Reformation] Time to return to default: {reform_duration:.2f} s")
            in_nondefault_formation = False
        ...
        metrics["cpu_loads"].append(cpu_time)

        # Path length tracking
        if positions_prev:
            for i, (curr, prev) in enumerate(zip(safe_next_positions, positions_prev)):
                metrics["path_lengths"][i] += distance(curr, prev)
        positions_prev = safe_next_positions
            

    print("Decentralized Stage 3 mission complete. Landing swarm.")
    time1 = time.time()
    metrics["end_time"] = time1
    ...
    avg_reform = np.mean(metrics["reformation_times"]) if metrics["reformation_times"] else 0.0
    avg_path = np.mean(metrics["path_lengths"])
    avg_cpu = np.mean(metrics["cpu_loads"])
    ...
    print(f"Formation switches: {metrics['formation_switch_count']}")
    print(f"Average reformation time: {avg_reform:.2f} seconds")
    print(f"Average path length: {avg_path:.2f} meters")
    print(f"Average CPU load: {avg_cpu:.3f} seconds per step")
    print(f"Per-drone path lengths: {[round(length, 2) for length in metrics['path_lengths']]} meters")
    print(f"Max CPU load: {max(metrics['cpu_loads']):.3f} seconds")

    results_dir = "results"
    if not os.path.exists(results_dir):
        os.makedirs(results_dir)

    with open(f"{results_dir}/stage3_dec.txt", "a") as f:
        f.write(f"-------------------------------------------------------------------\n")
        f.write(f"Stage 3: Decentralized Forest Traversal\n")
        f.write(f"Total time: {total_time:.3f} sec\n")
        f.write(f"Formation switches: {metrics['formation_switch_count']}\n")
        f.write(f"Average reformation time: {avg_reform:.3f} sec\n")
        f.write(f"Average path length: {avg_path:.3f} m\n")
        f.write(f"Average CPU load: {avg_cpu:.3f} s/step\n")
        f.write(f"Per-drone path lengths: {[round(length, 3) for length in metrics['path_lengths']]} m\n")
        f.write(f"Max CPU load: {max(metrics['cpu_loads']):.3f} s\n")
